...
class SyndromeBuilderChain(BuilderChainABC):
    def build(self, **kwargs):
        source = CSVReader(output_type="Chunk", id_col="idx", name_col="title", content_col="text")
        splitter = LengthSplitter(split_length=2000)
        extractor = KAGExtractor()
...
def import_data():
    ...
    DefaultStructuredBuilderChain("Efficacy").invoke(file_path=os.path.join(file_path,"data/Efficacy.csv"))
    
    SyndromeBuilderChain().invoke(file_path=os.path.join(file_path,"data/Syndrome.csv"))
